...
python_script="/abga/work/andrgrig/MFD-ILP/MFD in DAGS/mfd_inexact.py"
...
for file in "$directory"/*; do
	if [ -f "$file" ] && [ "${file##*.}" = "graph" ]; then
        ...
        output_file="${file%.graph}.standard.out"

        # Run the Python script with the file as input and extract the runtime
		run_stdout=$( { time python3 "$python_script" -i "$file" -o "$output_file"; } 2>&1)

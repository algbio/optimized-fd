...
def build_ilp_model_avoiding_multiple_paths(data, size, paths):
    ...
    model, x, _, _ = build_base_ilp_model(data, 0, size)
...
def fd_fixed_size(data, paths, size):
    ...
    try:
        # Create a new model
        model, _, _, _ = build_base_ilp_model(data, paths, size)
...
def output_paths(output,paths):
    ...
    #output.write('-1 ')
    #output.write(' '.join(map(str, sorted(set([item for t in paths for item in t[:-1]])))))
    #output.write('\n')
...
def solve_instances(graphs, predefined_paths,output_file, output_stats=False, use_excess_flow=False,
    ...
    for g, graph in enumerate(graphs):
        ...
        known_paths = predefined_paths[g]
        ...
        if len(mfd['graph'].edges) > 0:
            ...
            try:
                mfd = mfd_algorithm(mfd,known_paths)

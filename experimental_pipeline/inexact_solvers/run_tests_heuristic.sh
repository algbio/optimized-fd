...
python_script="/abga/work/andrgrig/ifd_package/run_sc_experiments.py"
...
for file in "$directory"/*; do
	if [ -f "$file" ] && [ "${file##*.}" = "graph" ]; then
        ...
        output_file="${file%.graph}.heuristic.out"
		predicted_file="${file%.graph}.predicted"
		> $predicted_file

        # Run the Python script with the file as input and extract the runtime
		run_stdout=$( { time python3 "$python_script"  "$file" > "$output_file"; } 2>&1)
		...
		# Extract the returned number of paths in output_file of IFD solver
		k=$(grep -n "Weights are:" "$output_file" | awk -F',' '{print NF}')

...
for file in "$directory"/*; do
	if [ -f "$file" ] && [ "${file##*.}" = "graph" ]; then
		...
		orig_heuristic_file="${file%.graph}.predicted" 
		heuristic_file="${file%.graph}.heuristic_for_opt.out" 
		...
		run_stdout=$( { time python3 "$python_script" -i "$file" -o "$output_file" --heuristic "$heuristic_file" --inexact --verbose; } 2>&1)
		...
		sol_length_case=$(echo "$run_stdout" | grep "Number of edges in the MFD solution:" | awk '{print $8}')
		sol_length=$(echo "$sol_length + $sol_length_case" | bc)
...
echo "Number of edges in the MFD solution: $sol_length."
